...
public:
	...
	Point(int id, ANNcoord x, ANNcoord y);
	Point(const Point&);
	~Point();
	static ANNpointArray ConvertToANNpointArray(const std::vector<Point>&points, int n);
};
...
public:
	...
	ANNcoord* low;    //lower bounds in each dimension
	ANNcoord* high;   //higher bounds in each dimension

	Polygon(std::vector<Point>& polygon_vertices, int m);
	~Polygon();
};
...
public:
	...
	std::vector<Point>& annRangeSearch(      // approx range search in polygon
		Polygon& polygon);              // input polygon shape for range search
	                                    // return size of points in polygon
